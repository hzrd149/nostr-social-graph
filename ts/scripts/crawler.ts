...
import {
  createScriptNostrContext,
  loadAddressPointer,
  type LoadableAddressPointer,
  type ScriptNostrContext,
} from "./nostr";
...
export class Crawler {
  ...
  constructor(
    ...
    this.batchIdleMs = parsePositiveInt(
      process.env.CRAWL_BATCH_IDLE_MS,
      DEFAULT_CRAWL_BATCH_IDLE_MS,
    );
    this.batchMaxMs = parsePositiveInt(
      process.env.CRAWL_BATCH_MAX_MS,
      DEFAULT_CRAWL_BATCH_MAX_MS,
    );
    this.batchConcurrency = Math.max(
      1,
      parsePositiveInt(
        process.env.CRAWL_BATCH_CONCURRENCY,
        DEFAULT_CRAWL_BATCH_CONCURRENCY,
      ),
    );
    this.batchDelayMs = parsePositiveInt(
      process.env.CRAWL_BATCH_DELAY_MS,
      DEFAULT_CRAWL_BATCH_DELAY_MS,
    );

    this.debouncedSave = debounce(async () => {
      const start = Date.now();
      console.log(
        `Starting social graph serialization … (${this.eventsSinceLastSave} new events)`,
      );
      ...
      try {
        ...
        fs.writeFile(SOCIAL_GRAPH_LARGE_BIN, Buffer.from(serialized), (err) => {
          if (err) {
            console.error("failed to serialize SocialGraph", err);
          } else {
            const dur = Date.now() - start;
            console.log(
              `Saved social graph (size: ${this.socialGraph.size().users} users) in ${dur} ms`,
            );
          }
        });
      } catch (e) {
  ...
  async initialize() {
    ...
    if (event) {
      ...
      console.log(
        "Removing",
        removedCount,
        "muted users not followed by anyone",
      );
      this.debouncedSave();
    } else {
  ...
  private async crawlSocialGraph(myPubKey: string, upToDistance?: number) {
    ...
    console.log(
      `Starting iterative crawl with distance limit: ${distanceLabel}`,
    );

    // Process each distance level sequentially, waiting for fetches to complete
    for (
      let currentDistance = 0;
      currentDistance < (upToDistance ?? Number.POSITIVE_INFINITY);
      currentDistance++
    ) {
      ...
      const usersAtDistance =
        this.socialGraph.getUsersByFollowDistance(currentDistance);
      ...
      console.log(
        `Distance ${currentDistance}: fetching ${toFetch.size} users' follow lists`,
      );
      ...
      console.log(
        `Distance ${currentDistance}: recalculating follow distances...`,
      );
    ...
    console.log(
      "All distances processed. Graph size:",
      this.socialGraph.size(),
    );
  ...
  private fetchUsersInBatches(
    ...
    return new Promise((resolve) => {
      ...
      const launchNext = () => {
        while (
          inFlight < this.batchConcurrency &&
          ...
          const remaining = Math.max(
            users.length - nextBatchIndex * this.batchSize,
            0,
          );
          console.log(
  ...
  private fetchBatch(
    ...
    return new Promise((resolve) => {
      ...
      const finish = (reason: string) => {
        ...
        console.log(
          `Batch ${batchNumber}/${totalBatches} finished (${reason}) – processed ${eventsInBatch} events`,
        );
        this.debouncedSave();
        resolve();
      };
