...
const Explore = ({ pubKey, selectedUser }: ExploreProps) => {
  ...
  const generateFollowDistances = (prevDistances: FollowDistance[] = []) => {
    return [0, 1, 2, 3, 4, 5]
      .map((d) => {
        ...
        if (
          prevDistance &&
          prevDistance.randomUsers.length >= RANDOM_USER_COUNT
        ) {
          randomUsers = prevDistance.randomUsers;
        } else {
          randomUsers = users
            .sort(() => 0.5 - Math.random())
            .slice(0, RANDOM_USER_COUNT);
        }
        return {
          distance: d,
          count: users.length,
  ...
  const debouncedSetFollowDistances = useCallback(
    throttle(() => {
      setFollowDistances((prevDistances) =>
        generateFollowDistances(prevDistances),
      );
    }, 1000),
    [],
  );
  ...
  useEffect(() => {
    ...
    const subs = missing.map((pubkey) =>
      eventStore.replaceable(3, pubkey).subscribe((event) => {
        if (!event) {
          return;
        }
  ...
  return (
    <div className="flex flex-col gap-4 p-4 rounded-xl bg-base-100">
      ...
      {followDistances.length === 0 && (
        <div className="flex p-4 items-center justify-center">None</div>
      )}
